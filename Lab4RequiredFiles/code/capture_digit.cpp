...
// // Add this at the very bottom of capture_digit.cpp

// void display_digit_on_leds(CaptureDevice& dev, int digit) {
//     // Safety check to ensure memory is mapped
//     if (!dev.initialized || !dev.virtual_base_regs) {
//         return; 
//     }
    
//     // Calculate the physical pointer for the LEDR register
//     auto regs_base = reinterpret_cast<std::uintptr_t>(dev.virtual_base_regs);
//     volatile unsigned int* led_ptr = reinterpret_cast<volatile unsigned int*>(regs_base + (LEDR_BASE & HW_REGS_MASK));
    
//     // The DE1-SoC has 10 Red LEDs (LEDR0 to LEDR9). 
//     // To use the "upper 4 bits" (LEDR9, 8, 7, 6), we mask the digit 
//     // to ensure it is 4 bits max (0-15), then shift it left by 6 positions.
//     *led_ptr = (digit & 0x0F) << 6;
// }