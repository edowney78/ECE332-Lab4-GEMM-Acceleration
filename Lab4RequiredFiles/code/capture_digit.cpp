    ...
    constexpr std::uintptr_t HEX_BASE   = 0x1000;
    ...
    bool init_capture_device(CaptureDevice& dev, int roi_w, int roi_h, int border_inset) {
        ...
        dev.hex_ptr = reinterpret_cast<volatile unsigned short*>(regs_base + (HEX_BASE & HW_REGS_MASK));
    ...
    GrayImage capture_roi_gray_on_button(CaptureDevice& dev, bool save_debug_bmp, const std::string& debug_prefix) {
        if (!dev.initialized || !dev.video_in_dma || !dev.key_ptr || !dev.hex_ptr || !dev.video_mem) {
